...
function monthAndDay(birthDate: string, now: Date): string {
  ...
  return `MONTH ${Math.max(0, months)}, DAY ${days}`;
}
  ...
  if (!home.baby) {
    return (
      <section className="flex flex-col gap-4">
        <h1 className="text-[1.6rem] leading-tight text-[var(--text-primary)]">
        ...
        <Link
          ...
          className="mt-2 inline-flex h-12 items-center justify-center rounded-[var(--radius-button-primary)] bg-[var(--brand-primary)] px-6 text-[0.95rem] text-[var(--text-accent-warm)]"
        >
          Add your baby
        </Link>
      ...
      <article className="rounded-3xl border border-[var(--border-card)] bg-[var(--card-primary)] px-5 py-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[0.82rem] leading-[1.5] text-[var(--text-secondary)]">
              Age-appropriate essentials for {firstName} at {baby.ageLabel}.
            </p>
          </div>
        </div>
        <Link
