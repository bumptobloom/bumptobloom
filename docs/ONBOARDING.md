...
Owns the repo, CI/CD, Supabase project, auth, schema and RLS, EAS builds and
store accounts, the fever rules engine, AI architecture and evals, and code
review across all pods.
...
Owns Home and Track screens (Joanna), Learn and Cart screens (Melvin), and every
...
Owns the Ask Edge Function — prompts, context building, validation, evals (Shaff
Had, Rehaan) — plus the app's data layer and Supabase queries (Sivathmika,
Sahasra), and fever-check logging and recommendations (Rasheed).
...
4. Clone, install, get `npx expo start` running and the app open in Expo Go on
   your own phone. Ask your pod lead for env values.
