...
T = [
    ...
    (1, "Service worker with Serwist: cache the app shell, make the app installable", "Rasheed Adebayo OYEWOLE", "Pod I", "High",
     "manifest.ts already exists from Melvin's scaffold, so this is the other half and it is self-contained. Add @serwist/next, precache the app shell, do not cache Supabase responses yet - your own week 5 offline task decides what gets cached. You are building the thing you later have to prove works in airplane mode."),
    (1, "Draft the Home and Learn data-layer signatures for the contract freeze", "Sahasra Miriyala", "Pod I", "High",
     "You build getHome() in week 2 and the Learn data layer in week 3. Write the function names and return shapes now, so they go into docs/API-CONTRACTS.md at Friday's freeze. Pod E builds screens against these before your code exists, so this week the shape matters more than the implementation."),
    ...
    (1, "DATA: Milestone dataset schema, validation rules and data dictionary", "Tarigopula Sivathmika Chowdary", "Pod I", "High",
     "Comes BEFORE more extraction. Decide the columns, the allowed values, and what makes a row invalid, then fill. Typing 20 cells into a spreadsheet and discovering afterwards that half have no source is the failure mode this prevents.", "Data"),
    ...
    (1, "Begin CDC milestone dataset extraction", "Tarigopula Sivathmika Chowdary", "Pod I", "High",
     "cdc.gov/act-early/milestones. 4 domains x 5 checkpoints for v1: 2, 6, 12, 18, 24 months. The other four CDC checkpoints are additive and get backfilled after the demo."),
    ...
    (2, "Migration 0004: temperature_readings table - BLOCKS VITALS", "Sonakshi Panda", "Pod W", "Critical",
     "STILL NOT WRITTEN as of 18 Sep, and it is the single thing standing between Joanna and the Vitals screen. Renumbered: 0002 is the private avatars bucket and 0003 is the prompt-version constraint, both already on main, so this lands as 0004. BLOCKS the Vitals work. The existing fever_checks table cannot store a plain reading: age_months_at_check, rectal_equivalent_f, red_flags, tier, rule_id and rules_version are all NOT NULL and the log fills none of them. It also has no notes column, and the Figma has a notes field. The method values differ too - the design offers Ear, Armpit, Forehead and Rectal, the table allows rectal, oral, axillary, temporal, tympanic. New temperature_readings table with baby_id, temp_f, method, notes, created_at, RLS on, owner-scoped. Leave fever_checks in place and unused rather than altering six NOT NULL constraints."),
    ...
    (6, "Final MVP walkthrough", "Sonakshi Panda", "Pod W", "High",
     "Requested by Katrina, 1 Sep. One end-to-end pass through the whole product on a real phone before the demo, with the PMs and designers watching, so the first time anyone outside engineering sees it joined up is not on 6 October. Write down what breaks and triage it the same day."),
    ...
    (4, "Recommended for You: product list and product detail screens", "Melvin James Bryant III", "Pod E", "High",
     "RENAMED from Cart. Cart is no longer a tab - Product confirmed 1 Sep. Entry is a Recommended for You card on Home, which opens a product list for the baby's age, and tapping a product opens a detail screen showing why it is recommended plus a Shop now button out to a third-party retailer. No checkout, no basket, no total. Keep the 'we only link to trusted retailers' line and the 'general suggestions, not medical advice' disclaimer that are both in the Figma."),
    ...
    (6, "Full E2E suite green across all five tabs", "Melvin James Bryant III", "Pod E", "High", ""),
    (6, "Triage guard final pass before launch", "Sonakshi Panda", "Pod W", "Critical",
     "RESCOPED. The fever half of the safety table ships nothing in v1, but the triage guard is still live and still the thing stopping Ask from answering a question about a sick baby. Final pass on the guard's tests, its refusal copy and Rehaan's precision and recall numbers before we put this in front of real parents."),
    ...
    (3, "Approve Learn content copy before it ships", "Jasdeep Singh", "Product", "High",
     "Sahasra builds the dataset; someone has to sign off that the copy is accurate and on-brand before it reaches parents. Check every card carries a source label too."),
