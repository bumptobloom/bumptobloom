...
CLOSE_COMMENT = (
    ...
    "We moved from Next.js to Expo / React Native because BumpToBloom ships to "
    "the App Store and Play Store — see `docs/DECISIONS.md`, ADR-005. Most of "
    "these have a direct replacement issue; check the board for the new one."
)
...
UI_WORDS = ("screen", "navigator", "banner", "tokens", "sidebar",
            "accessibility", "device pass", "assets")
...
def pr_checklist(name: str) -> list[str]:
    ...
    items = [
        ...
        "the card",
        "No secret in the diff — nothing beyond `EXPO_PUBLIC_*`",
        "Reviewed by someone outside your pod",
    ]
    if any(w in n for w in UI_WORDS):
        items += [
            "Run on a real phone, not only a simulator",
            "Screenshot attached, taken on the smallest phone we support",
